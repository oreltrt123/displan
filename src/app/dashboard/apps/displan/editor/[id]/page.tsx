...
export default function EditorPage() {
  ...
  const loadComments = async () => {
    // Skip loading comments for CMS pages
  ...
  const loadElements = async () => {
    // Skip loading elements for CMS pages in preview mode
    if (currentPage.startsWith("cms-") && isPreviewMode) {
      setElements([])
      return
    }
  ...
  const handleCreateComment = async (x: number, y: number, message: string) => {
    // Don't allow comments on CMS pages
    if (currentPage.startsWith("cms-")) {
      return
    }
  ...
  const handleTogglePreviewMode = () => {
    ...
    // Reload elements when toggling preview mode for CMS pages
    if (currentPage.startsWith("cms-")) {
      loadElements()
    }
  }
